...
The fake-server suite pins the response shapes above (including the accepted-but-dropped turn on an archived thread and the not-found after archive), token minting and refresh without leaking the token, the worktree-bound `thread.create`, the stop-then-archive close order, and the spawn, peek, control, and teardown paths.
